...
import type { ExpoRoutesManifestV1 } from "expo-router/build/routes-manifest.js";
import { ExpoRouterServerManifestV1FunctionRoute, RouteInfo } from "./types.js";
// @ts-ignore
import manifestJSON from "__STATIC_CONTENT_MANIFEST";

const assetManifest = JSON.parse(manifestJSON);
const routesManifest: ExpoRoutesManifestV1 = {
  apiRoutes: [],
  htmlRoutes: [],
  notFoundRoutes: [],
};
...
export function createRequestHandler({
  ...
  env,
  ctx,
  logApiRouteExecutionError = (error: Error) => { // TODO: default down below
    console.error("logApiRouteExecutionError", error);
  },
}: {
  routes: ExpoRoutesManifestV1,
  request: Request,
  env: any,
  ctx: any
